...
namespace Calculator
{
    class Program
    {
            ...
            while (true)
            {
                try
                {
                   ...
                   "Enter: ");
                    ...
                    while (equation.Contains('(') && equation.Contains(')'))/* Check if there are brackets and 
                 * elements before left brackect and by the right bracket  will be letters or no. If letter, throw exception, because we have proccesed all string operations */
                    {
                        ...
                        {
                            equation = equation.Replace("(" + tsq + ")", subEquation);
                            Console.WriteLine(tsq + " = " + subEquation + "\nNow equation is " + equation + "\n----------------------------------");
                        }
                    }
                    while (equation.Contains("*") || equation.Contains("/") || equation.Contains("+") || equation.Contains("-") || equation.Contains("^"))
                    {
                        ...
                        else if (subList.Count == 1)
                        {
                            ...
                            Console.WriteLine(tsq + " = " + equation + "\nNow equation is " + equation + "\n----------------------------------");
                            break;
                        }
                        if (equation.Contains("-") && equation.Split().Length < 3) break;
                    }
                    if (Regex.IsMatch(equation, @"^\d")) controlInput = false;
                ...
                {
                    Console.WriteLine("\n" + "EXCEPTION ERROR: " + new StackTrace().GetFrame(0).GetMethod().Name + ": " + ex.Message + "\n");
                }
            }
        }
        /// <summary>
        ...
        static string ExtractPow(string equation)
        {
            ...
            equation = equation.Insert(comaBetweenValues + 3, "(");
        ...
        static void validateequation(ref string equation, out List<string> resultarr)
        {
            ...
            if (resultarr.Count == 2 && resultarr[0] != "/" && resultarr[0] != "*")
            {
                resultarr[0] = resultarr[0] + resultarr[1];
                resultarr.RemoveAt(1);
            }
        ...
        static double Prioritiescalculation(ref string mathStr, ref List<string> mathParts)
        {
            ...
            Console.WriteLine("Length mathparts: " + mathParts.Count);
            if (mathStr.Contains("^"))
            {
                Isonenumber = false;
                for (int i = 1; i < mathParts.Count; i++)
                {
                    ...
                    if (mathParts[i] == "^")//If element is multiplier or diviser, we get operand and neighboring elements to make calculations
                    {
                        ...
                        Console.WriteLine("Math parts: ");
                        mathParts.ForEach(Console.WriteLine);
